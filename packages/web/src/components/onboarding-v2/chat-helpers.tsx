...
export function isSketchOrigin(msg: ChatMessage | undefined): boolean {
  if (!msg) return false;
...
export function renderRichMessage(text: string): ReactNode {
  const regex = /##(.+?)##|\[\[(.+?)\]\]|\{([^|]+?)\|([^}]+?)\}/g;
...
/** Detects if a sketch message contains rich markers. */
export function hasRichMarker(text: string | undefined): boolean {
  return !!text && (text.includes("##") || text.includes("[[") || /\{[^|]+\|[^}]+\}/.test(text));
}
